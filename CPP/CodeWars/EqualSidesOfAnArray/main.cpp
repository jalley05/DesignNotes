/*
    ...
    the sum of the integers to the left of N is equal to the sum of the integers to the right of N. If
...
int findEventIdx(const vector<int> numbers) 
{
    int index = -1;
    for(int ii = 0; ii < numbers.size(); ii++)
    {
        vector<int> leftSide(numbers.begin(), numbers.begin() + ii);
        vector<int> rightSide(numbers.begin() + ii + 1, numbers.end());

        if(accumulate(leftSide.begin(), leftSide.end(), 0) ==
            accumulate(rightSide.begin(), rightSide.end(), 0))
        {
            index = ii;
            break;
        }
...
int main() {
    ...
    int answer = findEventIdx(numbers);
    assert(3 == answer);
